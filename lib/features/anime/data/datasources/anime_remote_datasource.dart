...
abstract class AnimeRemoteDataSource {
  ...
  Future<List<AnimeModel>> getMostPopularAnimes(
  ...
  Future<List<AnimeModel>> getCurrentlyAiringAnimes(
      {int offset = 0, int limit = 10});
  Future<AnimeModel> getAnimeDetails(String id);
  Future<List<AnimeModel>> searchAnimes(String query,
      {int offset = 0, int limit = 10});
  Future<List<AnimeModel>> getAnimesByCategory(String categorySlug,
      {int offset = 0, int limit = 10});
}
...
class AnimeRemoteDataSourceImpl implements AnimeRemoteDataSource {
  ...
  Future<List<AnimeModel>> getTrendingAnimes(
    ...
    return _fetchList(
        '/trending/anime?page[limit]=$limit&page[offset]=$offset');
  }
  ...
  Future<List<AnimeModel>> getMostPopularAnimes(
      {int offset = 0, int limit = 10}) async {
    return _fetchList(
        '/anime?sort=-userCount&page[limit]=$limit&page[offset]=$offset');
  }

  @override
  Future<List<AnimeModel>> getTopRatedAnimes(
      {int offset = 0, int limit = 10}) async {
    return _fetchList(
  ...
  @override
  Future<List<AnimeModel>> getUpcomingAnimes(
      {int offset = 0, int limit = 10}) async {
    return _fetchList(
  ...
  Future<List<AnimeModel>> getCurrentlyAiringAnimes(
      {int offset = 0, int limit = 10}) async {
    return _fetchList(
        '/anime?filter[status]=current&page[limit]=$limit&page[offset]=$offset');
  }
  ...
  Future<List<AnimeModel>> searchAnimes(String query,
      {int offset = 0, int limit = 10}) async {
    return _fetchList(
        '/anime?filter[text]=$query&page[limit]=$limit&page[offset]=$offset');
  }
  ...
  Future<List<AnimeModel>> getAnimesByCategory(String categorySlug,
      {int offset = 0, int limit = 10}) async {
    return _fetchList(
        '/anime?filter[categories]=$categorySlug&page[limit]=$limit&page[offset]=$offset');
  }
