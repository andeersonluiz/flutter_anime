...
class AnimeInfoPage extends StatefulWidget {
  ...
  final actualBar;
  final isSearch;
  AnimeInfoPage(this.anime, this.index, this.actualBar, this.isSearch);
...
class _AnimeInfoPageState extends State<AnimeInfoPage> {
  ...
  void didChangeDependencies() {
    ...
    Timer.run(() {
      if (storeEpisodes.listEpisodes != null) {
        storeEpisodes.listEpisodes = null;
      }
      if (storeTranslation.synopsisTranslated != null) {
              ...
              actions: [
                Observer(builder: (_) {
                  return widget.isSearch
                      ? IconButton(
                          icon: Icon(
                            storeSearch.favStatus[widget.index][1]
                                ? Icons.star
                                : Icons.star_border,
                            color: Colors.yellow,
                            size: height * 0.045,
                          ...
                          onPressed: () {
                            if (firebaseStore.isLogged) {
                              if (storeSearch.favStatus[widget.index][1]) {
                                storeAnimes
                                    .removeFavoriteByName(widget.anime.id);
                              } else {
                                storeAnimes.addFavoriteByName(widget.anime.id);
                              }
                              firebaseStore.setFavorite(widget.anime,
                                  storeSearch.favStatus[widget.index][1]);
                              storeSearch.changeStatus(widget.index);
                            } else {
                              return Toast.show(
                      ...
                      : widget.actualBar == globals.stringAnimesPopular
                          ? IconButton(
                              icon: Icon(
                              ...
                              onPressed: () {
                                if (firebaseStore.isLogged) {
                                  firebaseStore.setFavorite(
                                      widget.anime,
                                      storeAnimes
                                              .favoriteListPopular[widget.index]
                                          [1]);
                                  storeAnimes
                                      .setfavoriteListPopular(widget.index);
                                  storeSearch.changeStatusById(widget.anime);
                                } else {
                                  return Toast.show(
                                      translate('anime_info.error_favorite'),
                                      context,
                          ...
                          : widget.actualBar == globals.stringAnimesAiring
                              ? IconButton(
                                  icon: Icon(
                                    storeAnimes.favoriteListAiring[widget.index]
                                            [1]
                                        ? Icons.star
                                        : Icons.star_border,
                                    color: Colors.yellow,
                                    ...
                                    if (firebaseStore.isLogged) {
                                      firebaseStore.setFavorite(
                                          ...
                                          storeAnimes.favoriteListAiring[
                                              widget.index][1]);
                                      storeAnimes
                                          .setfavoriteListAiring(widget.index);
                                      storeSearch
                                          .changeStatusById(widget.anime);
                                    } else {
                                      return Toast.show(
                                          translate(
                              ...
                              : widget.actualBar == globals.stringAnimesHighest
                                  ? IconButton(
                                      icon: Icon(
                                      ...
                                      onPressed: () {
                                        if (firebaseStore.isLogged) {
                                          firebaseStore.setFavorite(
                                              ...
                                              storeAnimes.favoriteListHighest[
                                                  widget.index][1]);
                                          storeAnimes.setfavoriteListHighest(
                                              widget.index);
                                          storeSearch
                                              .changeStatusById(widget.anime);
                                        } else {
                                  ...
                                  : widget.actualBar ==
                                            ...
                                            if (firebaseStore.isLogged) {
                                              firebaseStore.setFavorite(
                                                  ...
                                                  storeAnimes
                                                      ...
                                                      widget.index][1]);
                                              storeAnimes
                                                  .setfavoriteListPopular(
                                                      widget.index);
                                              storeSearch.changeStatusById(
                                                  widget.anime);
                                            } else {
                                              return Toast.show(
                                                  translate(
                                      ...
                                      : IconButton(
                                          icon: Icon(
                                            storeAnimesCategories.favCategorie[
                                                    widget.index][1]
                                                ? Icons.star
                                                : Icons.star_border,
                                            color: Colors.yellow,
